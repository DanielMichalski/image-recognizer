...
public class ProcessEngine {
        ...
        for (int y = 0; y < stencil.length; ++y)
            for (int x = 0; x < stencil[y].length; ++x) {
                if (stencil[y][x]) {
                    ...
                    centerPoints.add(new Integer[]{top_y + (bottom_y - top_y) / 2,
    ...
    public int[] shapeRecognize(Map<String, LinkedList<TreeSet<Integer[]>>> map,
        ...
        for (Map.Entry<String, LinkedList<TreeSet<Integer[]>>> entry : map.entrySet()) {
            ...
            if (shape != ShapePreview.Shape.TRIANGLE) {
                ...
                graphicsObject.setPaint(Color.GREEN);
                graphicsObject.drawLine(A_x + distance_x, A_y + distance_y,
                        A_x + distance_x, A_y + distance_y);
                graphicsObject.drawLine(B_x, B_y + distance_y, B_x, B_y + distance_y);
                graphicsObject.drawLine(C_x - distance_x, C_y + distance_y,
                        C_x - distance_x, C_y + distance_y);
                graphicsObject.drawLine(E_x - distance_x, E_y - distance_y,
                        E_x - distance_x, E_y - distance_y);
                graphicsObject.drawLine(F_x, F_y - (F_y - center_y) / 2,
                        F_x, F_y - (F_y - center_y) / 2);
                graphicsObject.drawLine(G_x + distance_x, G_y - distance_y,
                        G_x + distance_x, G_y - distance_y);
            }
            switch (shape) {
                case BOX:
                    if (matrix[A_y + delta][A_x + delta] &&
                        ...
                        ++found;
                        graphicsObject.setPaint(Color.RED);
                        graphicsObject.drawLine(center_x - delta, center_y - delta,
                ...
                case CIRCLE:
                    ...
                    break;
                case TRIANGLE:
                    if (matrix[A_y + distance_y][A_x + distance_x] &&
    ...
    private void rotate() {
        ...
        AT_operation = new AffineTransformOp(transform,
